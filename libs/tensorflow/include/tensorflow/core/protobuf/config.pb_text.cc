...
void AppendProtoDebugString(
  ...
  o->AppendBoolIfTrue("client_handles_error_formatting", msg.client_handles_error_formatting());
...
bool ProtoParseFromScanner(
  ...
  while(true) {
    ...
    else if (identifier == "client_handles_error_formatting") {
      if (has_seen[1]) return false;
      has_seen[1] = true;
      bool value;
      if (!parsed_colon || !::tensorflow::strings::ProtoParseBoolFromScanner(scanner, &value)) return false;
      msg->set_client_handles_error_formatting(value);
    }
